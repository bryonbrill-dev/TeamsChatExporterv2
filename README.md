...
- **`AADSTS7000218` / `invalid_client` / missing `client_secret`**:
  - In your Entra app registration, go to **App registrations** → **your app** → **Authentication**.
  - Under **Advanced settings**, set **Allow public client flows** = **Yes**.
  - Save, then retry the script.
  - The script uses OAuth device code flow and does **not** use a client secret.
