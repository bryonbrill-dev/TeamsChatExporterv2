...
<#
    ...
    .PARAMETER From
        Optional start date/time (inclusive) for filtering messages.

    .PARAMETER To
        Optional end date/time (inclusive) for filtering messages.
...
foreach ($thread in $chats) {
    ...
    if (($conversation.count -gt 0) -and (-not([string]::isNullorEmpty($name)))) {
        ...
        foreach ($message in $conversation) {
            $messageTime = [System.TimeZoneInfo]::ConvertTimeBySystemTimeZoneId((Get-Date ($message.createdDateTime)), (Get-TimeZone).Id)
            if ($null -ne $From -and $messageTime -lt $From) {
                continue
            }
            if ($null -ne $To -and $messageTime -gt $To) {
                continue
            }
            ...
            $time = Get-Date $messageTime -Format "dd MMMM yyyy, hh:mm tt"
        ...
        if ($hasMessagesInRange -and -not [string]::IsNullOrWhiteSpace($messagesHTML)) {
            ...
            Write-Host ($name + " :: No messages found in range.")
            Write-Host -ForegroundColor Yellow "Skipping...`r`n"
        }
    }
    else {
